...
struct Header {
	...
	uint8_t leader_size_bytes{};
	uint8_t follower_size_bytes{};
	uint8_t counter_size_bytes{};
	uint8_t barcode_len_symbols{};
	uint8_t leader_len_symbols{};
	uint8_t follower_len_symbols{};
	uint8_t gap_len_symbols{};

	uint32_t rec_len{};

	enum class ordering_t {SBLFC = 0, LFSBC = 1, FLSBC = 2};

	inline static const ordering_t default_ordering = ordering_t::SBLFC;

	static std::string to_string(ordering_t ordering)
	{
		switch (ordering)
		{
		case ordering_t::LFSBC:
			return "LFSBC";
		case ordering_t::SBLFC:
			return "SBLFC";
		case ordering_t::FLSBC:
			return "FLSBC";
		default:
			assert(false);
		}
	...
	{
		if (ordering_str == "LFSBC")
			return ordering_t::LFSBC;
		if (ordering_str == "SBLFC")
			return ordering_t::SBLFC;
		if (ordering_str == "FLSBC")
			return ordering_t::FLSBC;

		std::cerr << "Error: unknown ordering: " << ordering_str << "\n";
		exit(1);
	}

	ordering_t ordering = ordering_t::SBLFC;

	bool operator==(const Header& rhs) const {
		return sample_id_size_bytes == rhs.sample_id_size_bytes &&
			barcode_size_bytes == rhs.barcode_size_bytes &&
			leader_size_bytes == rhs.leader_size_bytes &&
			follower_size_bytes == rhs.follower_size_bytes &&
			counter_size_bytes == rhs.counter_size_bytes &&
			barcode_len_symbols == rhs.barcode_len_symbols &&
			leader_len_symbols == rhs.leader_len_symbols &&
			follower_len_symbols == rhs.follower_len_symbols &&
	...
	void serialize(buffered_binary_writer& out) const {
		...
		out.write_little_endian(leader_size_bytes);
		out.write_little_endian(follower_size_bytes);
		out.write_little_endian(counter_size_bytes);
		out.write_little_endian(barcode_len_symbols);
		out.write_little_endian(leader_len_symbols);
		out.write_little_endian(follower_len_symbols);
	...
	void load(buffered_binary_reader& in) {
		...
		in.read_little_endian(leader_size_bytes);
		in.read_little_endian(follower_size_bytes);
		in.read_little_endian(counter_size_bytes);
		in.read_little_endian(barcode_len_symbols);
		in.read_little_endian(leader_len_symbols);
		in.read_little_endian(follower_len_symbols);
		in.read_little_endian(gap_len_symbols);

		rec_len = sample_id_size_bytes + barcode_size_bytes + leader_size_bytes + follower_size_bytes + counter_size_bytes;
	}

	void print(std::ostream& oss) const {
		const std::string ordering_name[] = { "SBLFC", "LFSBC", "FLSBC" };

		oss << "Header: \n";
		oss << "\tsample_id_size_bytes     : " << (uint64_t)sample_id_size_bytes << "\n";
		oss << "\tbarcode_size_bytes       : " << (uint64_t)barcode_size_bytes << "\n";
		oss << "\tleader_size_bytes        : " << (uint64_t)leader_size_bytes << "\n";
		oss << "\tfollower_size_bytes      : " << (uint64_t)follower_size_bytes << "\n";
		oss << "\tcounter_size_bytes       : " << (uint64_t)counter_size_bytes << "\n";
		oss << "\tbarcode_len_symbols      : " << (uint64_t)barcode_len_symbols << "\n";
		oss << "\tleader_len_symbols       : " << (uint64_t)leader_len_symbols << "\n";
		oss << "\tfollower_len_symbol      : " << (uint64_t)follower_len_symbols << "\n";
...
inline uint64_t pack_smaple_id_follower(uint64_t sample_id, uint64_t barcode) {
	return (sample_id << 32) + barcode;
}

inline void unpack_sample_id_follower(uint64_t packed, uint64_t& sample_id, uint64_t& barcode) {
	sample_id = packed >> 32;
	barcode = packed & ((1ull << 32) - 1);
}
inline std::pair<uint64_t, uint64_t> unpack_sample_id_follower(uint64_t packed) {
	std::pair<uint64_t, uint64_t> res;
	unpack_sample_id_follower(packed, res.first, res.second);
	return res;
}
...
private:
	void serialize_msb(std::vector<uint8_t>& out, const Header& header) {
		...
		{
		case Header::ordering_t::SBLFC:
			append_int_msb(out, sample_id, header.sample_id_size_bytes);
			append_int_msb(out, barcode, header.barcode_size_bytes);
			append_int_msb(out, leader, header.leader_size_bytes);
			append_int_msb(out, follower, header.follower_size_bytes);
			append_int_msb(out, count, header.counter_size_bytes);
			break;
		case Header::ordering_t::LFSBC:
			append_int_msb(out, leader, header.leader_size_bytes);
			append_int_msb(out, follower, header.follower_size_bytes);
			append_int_msb(out, sample_id, header.sample_id_size_bytes);
			append_int_msb(out, barcode, header.barcode_size_bytes);
	...
	void load_msb(std::vector<uint8_t>& in, const Header& header) {
		...
		case Header::ordering_t::SBLFC:
			...
			LoadBigEndian(p, leader, header.leader_size_bytes);
			LoadBigEndian(p, follower, header.follower_size_bytes);
...
public:
	...
	void print(std::ostream& oss, const Header& header, RecFmt format = RecFmt::SATC, const SampleNameDecoder& sample_name_decoder = SampleNameDecoder("")) {
		if (format == RecFmt::SATC) {
			...
			oss << kmer_to_string(leader, header.leader_len_symbols) << "\t";
			oss << kmer_to_string(follower, header.follower_len_symbols) << "\t";
			oss << count << "\n";
		}
